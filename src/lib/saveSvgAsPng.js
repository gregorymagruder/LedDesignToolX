...
(function () {
  ...
  out$.saveSvgAsPng = function (el, name, options) {
    requireDomNode(el);
    return out$.svgAsPngUri(el, options || {}, function (uri) {
      // return out$.download(name, uri);
      console.dir("Filename: " + name);

      let base64Data = uri.replace(/^data:image\/png;base64,/, "");
      base64Data += base64Data.replace('+', '');
      let binaryData = new Buffer(base64Data, 'base64').toString('binary');

      fs.writeFileSync(name, binaryData, "binary", (err) => {
        if (err) {
          alert("An error ocurred creating the PNG file step" + err.message)
        }
      });
